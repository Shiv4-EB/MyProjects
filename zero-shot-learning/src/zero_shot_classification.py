...
# keep this here for illustration
#############################################
""""
premise = sequences
hypothesis_template = 'This example is {}.'

hypothesis = hypothesis_template.format(labels[0])

# run through model pre-trained on MNLI
x = tokenizer.encode(premise[0], hypothesis, return_tensors='pt')
...
class TruncationStrategy(ExplicitEnum):
    """
    Possible values for the `truncation` argument in PreTrainedTokenizer.__call__. Useful for tab-completion in
    an IDE.
    """
...
class ArgumentHandler:
    ...
    How to:
    -------
    arghdler = ArgumentHandler()
    args,_ = arghdler(sequences, labels, hypothesis_template)
    print(args)
    """
...
class ZeroShotClassification(ChunkPipeline):
    """
    NLI-based zero-shot classification using a `ModelForSequenceClassification` trained on NLI (natural
    ...
    def __init__(self, args_parser=ArgumentHandler(), *args, **kwargs):
    ...
    def _parse_and_tokenize(
        self, sequence_pairs, padding=True, add_special_tokens=True, truncation=TruncationStrategy.ONLY_FIRST.value, **kwargs
    ):
        """
    ...
    def _sanitize_parameters(self, **kwargs):
        #I keep multi_label sanitizer for reference only
